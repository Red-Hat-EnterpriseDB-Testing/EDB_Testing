-- Run on the EDB Postgres for Kubernetes primary for namespace edb-postgres (e.g. pod postgresql-1) as superuser.
-- Replace REPLACE_WITH_STRONG_PASSWORD before execution (avoid characters that break shell/YAML: use a long random string).
--
-- Example:
--   oc exec -n edb-postgres -it postgresql-1 -- psql -U postgres -v ON_ERROR_STOP=1 -c "$(cat create-aap-databases.sql)"
