...
Example inventory and EE build files are in this collection: `inventory/example-multi-datacenter.yml`, `execution-environment.yml`, `requirements-ee.yml`. See [docs/GETTING_STARTED.md](docs/GETTING_STARTED.md) and [Execution environment (AAP)](#execution-environment-aap) below.
...
To run playbooks on Ansible Automation Platform (AAP), build an execution environment that includes this collection.

1. **Prerequisites**: [ansible-builder](https://ansible-builder.readthedocs.io/), Podman or Docker.
2. **Build collection tarball** (from this collection directory):
   ```bash
   cd ansible_collections/edb/postgres_operations
   ansible-galaxy collection build .
   ```
3. **Edit `requirements-ee.yml`**: uncomment the `edb.postgres_operations` entry and set `path` to the tarball (e.g. `edb-postgres_operations-1.1.0.tar.gz`).
4. **Build EE image**:
   ```bash
   ansible-builder build --tag edb-postgres-operations-ee:1.0
   ```
5. **Push** to your registry and add the EE in AAP (Administration → Execution Environments).
6. In job templates, set **Playbook** to e.g. `ansible_collections/edb/postgres_operations/playbooks/deploy-cluster.yml` (project root = repo root).

See `execution-environment.yml`, `requirements-ee.yml`, and `requirements.txt` in this directory.
...
- [GETTING_STARTED.md](docs/GETTING_STARTED.md) — Install and first playbook run
- [QUICK_REFERENCE.md](docs/QUICK_REFERENCE.md) — Commands and variables
- [SUMMARY.md](docs/SUMMARY.md) — Collection layout
- [COLLECTION_MIGRATION.md](docs/COLLECTION_MIGRATION.md) — Legacy → collection
